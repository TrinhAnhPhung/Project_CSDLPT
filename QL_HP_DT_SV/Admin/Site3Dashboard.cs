...
{
    public partial class Site3Dashboard : Form
    {
        private readonly string _connStr;
        private readonly string _allowedKhoa = "Business Administration"; // Chỉ cho phép truy vấn khoa này

        public Site3Dashboard()
        {
            InitializeComponent();
            ...
            _connStr = !string.IsNullOrWhiteSpace(Session.ConnectionString)
                ...
                : ConfigurationManager.ConnectionStrings["MyDB_Site3"]?.ConnectionString;

            if (string.IsNullOrWhiteSpace(_connStr))
            {
                MessageBox.Show("Connection string not found for Site3.",
        ...
        /// <summary>
        /// Load sinh viên khoa Business Administration từ Site3
        /// </summary>
        private async Task LoadStudentsAsync()
        {
            try
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    dt = await MultiSiteService.GetStudentsByKhoaFromSiteAsync(_connStr, "Quản trị kinh doanh");
                }
                ...
                if (dt.Rows.Count == 0)
                {
                    MessageBox.Show($"No students found for {_allowedKhoa} in Site3.", "Information",
            ...
            using (var searchForm = new SearchStudentForm())
            {
                if (searchForm.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        {
                            allStudents = await MultiSiteService.GetStudentsByKhoaFromSiteAsync(_connStr, "Quản trị kinh doanh");
                        }
